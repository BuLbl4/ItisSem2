...
const Header = () => {
    ...
    useEffect(() => { 
        if(fetching){
            axios.get(`http://localhost:5022/api/Pokemon/Pagination/${offset}`)         
                .then(data => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://localhost:5022/api/Pokemon/poke');
                    ...
                    {filterExecuted && filter.length === 0 && searchValue.length > 0 ? (
                        ...
                        filter.length > 0 ? (
                            filter.map((pokemon, index) => (
                                <PokemonCard key={index} pokemon={pokemon.name} />
                            ))
                        ) : (
